<template>
    ...
    <Dialog v-model:visible="isCreateModal" :style="{ width: '1000px' }" :breakpoints="{'960px': '75vw', '640px': '90vw'}"
        ...
        <div class="field">
            ...
            <Textarea :placeholder="$t('web.note')" rows="3" class="pt-1" type="text" v-model="formData.note" maxlength="100"></Textarea>
        </div>
        <div class="field">
        ...
        <template #footer>
            ...
            <Button v-if="!selectedBlock" :label="$t('common.add')" icon="pi pi-check" class="p-button p-component p-button-success"
                    @click="addBlock"/>
        </template>
    </Dialog>
</template>
