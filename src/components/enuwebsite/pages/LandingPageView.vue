<template>
  <div class="col-12">
    <TitleBlock :showBackButton="true"
                :title="$t('web.menuMainPage') + (pageData ? ' | ' + pageData['title_' + $i18n.locale] : '')"/>
    <BlockUI v-if="haveAccess" :blocked="loading">
      <div class="card">
        <Button :label="$t('web.addBlock')" @click="toggle"/>
      </div>
      <div class="card">
        <DataTable :lazy="true" :value="pageBlocks" :rows="10" dataKey="id" :rowHover="true" :loading="loading" @rowReorder="onRowReorder"
  ...
  <OverlayPanel ref="op" appendTo="body" :showCloseIcon="true" id="overlay_panel"
    ...
    <div class="p-fluid">
      <div class="field">
        ...
        <Dropdown v-model="selectedBlock" :options="blocks" optionLabel="name"
...
export default {
  ...
  setup() {
    ...
    const getBlocks = () => {
      ...
      enuService.getBlockList({slug: slug.value.slug}).then(res => {
        if (res.data) {
          ...
          if (blocks.value) {
            blocks.value.map(e => {
              e.name = e.note ? `${e['title_' + i18n.locale.value]} (${e.note})` : e['title_' + i18n.locale.value]
            })
          }
        }
      }).catch(error => {
    ...
    return {
      loading, pageData, blocks, selectedBlock, op, pageBlocks, haveAccess,
