  ...
  <Dialog v-model:visible="editMenuVisible" :style="{ width: '1000px' }"
    ...
    <div class="field" v-if="menuType === 1">
      <label for="choose-page">{{ $t("web.selectMainPage") }}
        <a href="javascript:void(0)" @click="showAddPage" class="ml-2 text-underline">{{
            $t('common.createNew')
          }}</a>
      </label>
    ...
    <div class="field">
      ...
      <FindUser v-model="formData.responsible_user" :max="1" searchMode="local" :user-type="3" />
    </div>
    ...
    <div class="field" v-if="formData.is_middle || formData.is_header">
      ...
      <div class="container1" v-if="removeIconBtn && formData.icon.length > 0">
        <div class="close-button" @click="removeIcon" v-if="formData.icon.length > 0">
                    <span id="pv_id_22_badge" :title="$t('hdfs.remove')" class="p-badge p-component p-badge-no-gutter"
                          style="background-color: #2b89d1 !important;" data-pc-name="badge" data-pc-section="root">
    ...
    getMenus(parentData) {
      ...
      this.enuService.getMenusTree(this.lazyParams).then(res => {
        if (parentData == null) {
          ...
          parentData.children = parentData.children ? parentData.children.filter(x => !x.link) : parentData.children;
