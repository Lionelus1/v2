<template>
  <div class="col-12">
    ...
    <!-- BEGINNING OF TABLE -->
    <ToolbarMenu :data="menu" @search="getAllEvents" :search="true"/>
    <!-- BEGINNING OF TABLE -->
    <div class="card">
      <DataTable :lazy="true" :value="allEvents" @page="onPage($event)" :totalRecords="eventsCount" :paginator="true"
...
export default {
  ...
  methods: {
    ...
    getAllEvents(data) {
      ...
      this.eventService.getEvents(this.lazyParams).then((response) => {
        if (response.data && response.data.events) {
          ...
          this.eventsCount = response.data.total;
        }
        this.loading = false;
      }).catch((error) => {
        this.loading = false;
