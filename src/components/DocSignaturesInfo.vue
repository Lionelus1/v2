  ...
  <div v-if="!loading">
    ...
    <TabView v-model:activeIndex="active" @tab-change="tabChanged">
      ...
      " :label="$t('common.downloadSignaturesPdf')" icon="pi pi-download" @click="downloadSignatures" class="p-button ml-2"/>
          <SignatureQrPdf ref="qrToPdf" :showSign="showSign()" :signatures="signatures" :title="docInfo.name" :approvalStages="approvalStages"
                          @downloadCMS="downloadCMSFile"></SignatureQrPdf>
        </div>
      ...
      || docInfo.docType === Enum.DocType.DocTemplate || docInfo.docType === Enum.DocType.DT_Request || docInfo.docType === Enum.DocType.ScienceWorksList)"
                :header="$t('common.revision')" :disabled="hideDocRevision">
        <div class="card">
...
export default {
  ...
  methods: {
    ...
    getData() {
      this.loading = true;
      api.post(`/agreement/getSignInfo`, {doc_uuid: this.doc_id,},
        ...
        if (res.data) {
          ...
          if (this.showAllSignsParam) {
            ...
            this.isShow =
                ...
                (this.signatures &&
                    this.signatures.some((x) => x.userId === this.loginedUserId)) ||
