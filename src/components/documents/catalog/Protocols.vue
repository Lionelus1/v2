<template>
  <!-- {{ currentDocument?.uuid }} -->
    ...
    <div v-if="docType === this.Enum.DocType.WorkPlanProtocolExtract" @click="goBackWithReload()" style="cursor: pointer;">
      <i class="fa-solid fa-arrow-left" style="float: left; margin-right: 10px;margin-top: 7px;font-size: 16px;"></i>
      <TitleBlock :title="$t('contracts.menu.registerProtocolExtracts')"  />
    </div>
    <div v-else>
      <TitleBlock :title="$t('contracts.menu.registerProtocols')" :show-back-button="true"/>
    </div>
    <ToolbarMenu :data="menu" @filter="toggle('filterOverlayPanel', $event)" :filter="true" style="max-height: 45px;" :filtered="filtered"/>
    <BlockUI :blocked="loading" class="card">
      ...
      <DataTable :value="documents" dataKey="id" :rows="rows" :totalRecords="total" :first="first"
        ...
        <Column :header="$t('contracts.columns.regDate')" style="min-width: 100px;">
          <template #body="slotProps">
            {{ slotProps.data.docHistory.setDate ? getShortDateString(slotProps.data.docHistory.setDate) : "" }}
          </template>
        </Column>
        <Column :header="$t('workPlan.protocol.briefDesc')">
            <template #body="slotProps">
              <div v-if="docType === this.Enum.DocType.WorkPlanProtocol">
                {{ getLocalizedProtocolType() }}
              </div>
              <div v-if="slotProps?.data?.docType === this.Enum.DocType.WorkPlanProtocolExtract">
                {{ $i18n.locale === 'en' ? 'Protocol extract' : $i18n.locale === 'ru' ? 'Выписка протокола' : 'Протокол үзіндісі' }}
              </div>
        ...
        <Column style="min-width: 50px;">
          <template #body="slotProps">
            <div class="flex flex-wrap">
              <!-- {{ slotProps?.data?.docHistory?.stateId !== Enum.CREATED.ID }} -->
  ...
  export default {
    ...
    data() {
      return {
        ...
        workPlanID: parseInt(this.$route.params.workPlanID),
      }
    },
    created() {
    ...
    methods: {
        ...
        getProtocolType() {
            let data = {
                protocol_code: this.Enum.ProtocolType.WorkPlanProtocol
            }
            this.planService.getProtocolType(data).then((res) => {
                if (res.data) {
                this.protocolType = res.data;
        ...
        getLocalizedProtocolType() {
            ...
            switch (locale) {
                case 'en':
                return this.protocolType.name_en;
                case 'kz':
                return this.protocolType.name_kz;
                case 'ru':
                return this.protocolType.name_ru;
                default:
                return this.protocolType.name_kz;
            }
        },
      ...
      goBackWithReload() {
        if(this.docType === this.Enum.DocType.WorkPlanProtocolExtract){
          this.$router.push({ 
            name: "Protocols", 
            params: { 
              docType: this.Enum.DocType.WorkPlanProtocol
            }
          }).then(() => {
            location.reload();
          });
      ...
      getRelatedDocs() {
        if(this.docType === this.Enum.DocType.WorkPlanProtocol){
          this.$router.push({ 
            name: "ProtocolExtracts", 
            params: { 
              docType: this.Enum.DocType.WorkPlanProtocolExtract, 
              workPlanID: this.currentDocument?.number 
            }
          }).then(() => {
            location.reload();
          });
        ...
        return [
          {
            ...
            icon: "pi pi-id-card",
            disabled: !this.currentDocument || this.currentDocument.docHistory?.stateId === Enum.CREATED.ID,
            command: () => {
          ...
          },
          {
            ...
            icon: "pi pi-file",
            disabled: !this.currentDocument || this.docType === Enum.DocType.WorkPlanProtocolExtract,//!this.currentDocument,
            visible: this.docType === Enum.DocType.WorkPlanProtocol,//this.findRole(null, RolesEnum.roles.ActsToExecution) || this.findRole(null, RolesEnum.roles.MainAdministrator) || this.findRole(null, RolesEnum.roles.ScienceDirector),
            command: () => {
              this.getRelatedDocs()
            },
          },
          {
            ...
            disabled: true, //!this.currentDocument || this.currentDocument.docHistory?.stateId !== Enum.APPROVED.ID || this.executed(this.currentDocument) || !this.execution(this.currentDocument),
            visible: false,//this.findRole(null, RolesEnum.roles.Accountant) || this.findRole(null, RolesEnum.roles.MainAdministrator),
            command: () => {
      ...
      actions() {
        return [
          
          {
            ...
            visible: this.actionsNode.docHistory && this.actionsNode.docHistory?.stateId !== Enum.CREATED.ID && this.actionsNode.docHistory?.stateId !== Enum.REVISION.ID,
            command: () => {
              this.currentDocument = this.actionsNode;
              this.open('documentInfoSidebar')
            },
          ...
          {
            label: this.$t('workPlan.protocol.previewDocument'),
