<template>
  <div v-if="!simpleUpload">
    <BlockUI v-if="uploading" :fullScreen="true"></BlockUI>
    <FileUpload name="demo[]" :multiple="true" :accept="accept" :maxFileSize="1000000" ref="ufile"
                @select="onSelectedFiles">
      <template #header="{ chooseCallback, clearCallback, files }">
        <div class="flex flex-wrap justify-content-between align-items-center flex-1 gap-2">
          <div class="flex gap-2">
            <Button @click="chooseCallback()" icon="pi pi-images" class="p-button-rounded"></Button>
            <Button @click="uploadRelatedDocs(clearCallback)" icon="pi pi-cloud-upload"
                    class="p-button-rounded p-button-success" :disabled="!files || files.length === 0"></Button>
            <Button @click="clearFiles(clearCallback)" icon="pi pi-times" class="p-button-rounded p-button-danger"
                    :disabled="!files || files.length === 0"></Button>
          </div>
          <ProgressBar v-if="uploading" mode="indeterminate" :showValue="false"
      ...
      <template #content="{ fileRemoveCallback }">
          ...
          <div class="flex flex-wrap p-5 gap-5">
            <div v-for="(file,index) of files" :key="file.name + file.type + file.size"
              ...
              <div class="align-items-center" style="width:75px;height:75px;">
                <img role="presentation" :alt="file.name" :src="file.objectURL" class="shadow-2"
                     style="max-width:100%;max-height:100%;"/>
              </div>
              <div>{{ formatSize(file.size) }}</div>
              <Badge :value="$t('common.pending')" severity="warning"/>
              <Button icon="pi pi-times" @click="onRemoveTemplatingFile(file, fileRemoveCallback, index)"
        ...
        <div v-if="uploadedFiles.length > 0">
          ...
          <div class="flex flex-wrap p-0 sm:p-5 sm:p-5 gap-5">
            <div v-for="file of uploadedFiles" :key="file.id"
                 class="card m-0 px-6 flex flex-column border-1 surface-border align-items-center gap-3">
              <div style="width:75px;height: 75px;" class="align-items-center">
                <img :ref="'img' + file.id" role="presentation" :alt="file.name" :src="imageUrl+file.filePath"
                     class="shadow-2" style="max-width: 75px;max-height: 75px;"/>
              </div>
              <Button icon="pi pi-check" @click="selectFile(file)"
                      class="p-button-outlined p-button-success p-button-rounded"/>
            </div>
          </div>
        </div>
      </template>
  ...
  <div v-else>
    <FileUpload name="demo[]" :accept="accept" :maxFileSize="1000000" ref="ufile"
                @select="onSelectedFiles">
      <template #header="{ chooseCallback, clearCallback, files }">
        <div class="flex flex-wrap justify-content-between align-items-center flex-1 gap-2">
          <div class="flex gap-2">
            <Button @click="chooseCallback()">{{ $t('common.choose') }}</Button>
            <Button @click="uploadRelatedDocs(clearCallback)"
                    class="p-button-success" :disabled="!files || files.length === 0">{{ $t('common.save') }}</Button>
            <Button @click="clearFiles(clearCallback)" class="p-button-danger"
                    :disabled="!files || files.length === 0">{{ $t('hdfs.remove') }}</Button>
          </div>
          <ProgressBar v-if="uploading" mode="indeterminate" :showValue="false"
                       :class="['md:w-20rem h-1rem w-full md:ml-auto', {'exceeded-progress-bar': (totalSizePercent > 100)}]">
            <span class="white-space-nowrap">{{ totalSize }}B / 1Mb</span></ProgressBar>
  ...
  methods: {
    ...
    uploadRelatedDocs(callback) {
      ...
      fd.append('info', JSON.stringify({
        directory: this.folder + "/" + this.fileID,
        count: fcount,
        fileID: this.fileID,
        uuid: this.uuid
      }));
