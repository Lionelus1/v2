      ...
      <div class="card p-0">
        <TabView ref="templateView" v-model:activeIndex="active" class="p-0">
          <TabPanel v-bind:header="$t('doctemplate.templates')" class="p-0">
            ...
            <Dialog :modal="true" v-bind:header="$t('doctemplate.newTemplate')" v-model:visible="dialogOpenState.addTemplate"
                    :style="{width: '50vw'}">
              <div class="p-fluid">
                <div class="field">
            ...
            <TreeTable :loading="loading" :value="templates" class="p-treetable-responsive p-treetable-sm" selectionMode="single"
                       @node-select="onNodeSelect" v-model:selectionKeys="currentNode" style="margin-bottom: 2rem">
              ...
              <Column headerStyle="width: 8em" headerClass="text-center" bodyClass="text-center">
                <template #header>
                  <Button v-if="!selectMode" icon="pi pi-plus" class="p-button-blue" @click="openForm('addFolder');clearCreatedFolder();"
                          v-tooltip.bottom="$t('common.newCatalog')"/>
                </template>
                <template #body="slotProps">
                  <Button v-if="slotProps.node.data.type==1 && !selectMode" type="button" icon="pi pi-plus"
                          @click="clearCreatedTemplate();initTemplateApprovalInfo(true, slotProps.node);" class="p-button-success"
                          v-tooltip.bottom="$t('doctemplate.newTemplate')"></Button>
                  <Button v-if="slotProps.node.data.type!=1" type="button" icon="pi pi-search" class="p-button-warning"
                          @click="editDocTemplate(slotProps.node)" v-tooltip.bottom="$t('common.show')"></Button>&nbsp;
                  <Button v-if="slotProps.node.data.type!=1 && slotProps.node.data.stateEn == DocState.REVISION.Value" icon="pi pi-comment"
                          @click="openForm('dialogComment',slotProps.node)" v-tooltip.bottom="$t('common.comment')"></Button>
                  <Button v-if="selectMode && slotProps.node.data.type!=1" icon="pi pi-check-circle" class="p-button-success ml-2"
                          @click="select($event,slotProps.node)" v-tooltip.bottom="$t('common.choose')"></Button>
          ...
          <TabPanel v-bind:header="$t('doctemplate.selected')" :disabled="(selectedNode.data.type == null || selectedNode.data.type == 1)">
            <div class="formgroup-inline">
              <span class="buttonset">
                <Button style="margin-right: 0.5rem;"
                        v-if="(selectedNode.data.stateEn == DocState.CREATED.Value || selectedNode.data.stateEn == DocState.REVISION.Value)"
                        :label="$t('common.save')" icon="pi pi-save" @click="saveDocTemplate"/>
                <Button style="margin-right: 0.5rem;" v-bind:label="$t('common.download')" icon="pi pi-file-pdf" @click="downloadDocTemplatePdf"/>
                <Button style="margin-right: 0.5rem;"
                        v-if="selectedNode.data.needApproval && ((selectedNode.data.stateEn == DocState.INAPPROVAL.Value && findRole(null, RolesEnum.roles.LegalServiceHead)) || selectedNode.data.stateEn == DocState.REVISION.Value || selectedNode.data.stateEn == DocState.APPROVED.Value)"
                        class="p-button-warning" :label="$t('doctemplate.approvalUsers')" icon="fa-solid fa-users-gear"
                        @click="initApprovalInfo(true);"></Button>
                <Button style="margin-right: 0.5rem;"
                        v-if="selectedNode.data.stateEn == DocState.CREATED.Value || selectedNode.data.stateEn == DocState.REVISION.Value"
                        :label="$t('common.toapprove')" icon="pi pi-send" @click="openForm('toApproval')"/>
                <Button style="margin-right: 0.5rem;"
                        v-if="selectedNode.data.stateEn == DocState.INAPPROVAL.Value && findRole(null, RolesEnum.roles.LegalServiceHead)"
                        class="p-button-success" :label="$t('common.approve')" icon="pi pi-check" @click="approve()"/>
                <Button style="margin-right: 0.5rem;"
                        v-if="selectedNode.data.stateEn == DocState.INAPPROVAL.Value && findRole(null, RolesEnum.roles.LegalServiceHead)"
                        class="p-button-warning" :label="$t('common.revision')" icon="pi pi-times" @click="openForm('revision')"/>
                <Button style="margin-right: 0.5rem;" v-if="selectedNode.data.stateEn == DocState.APPROVED.Value" class="p-button-primary"
                        :label="$t('common.approvalList')" icon="pi pi-user-edit" @click="openForm('signerInfo')"/>
              </span>
              ...
              <Button v-if="selectMode && selectedNode.data.type!=1" icon="pi pi-check-circle" class="p-button-success ml-2"
                      @click="select($event,selectedNode)" v-tooltip.bottom="$t('common.choose')"></Button>
            </div>
            ...
            <Dialog :header="$t('doctemplate.approvalUsers')" v-model:visible="dialogOpenState.approvalUsers"
              ...
              <div class="field">
                <ApprovalUsers :approving="approving" :mode="'doc_template'" v-model="currentApprovalStages"
                               :readonly="selectedNode.data.stateEn == DocState.INAPPROVAL.Value || selectedNode.data.stateEn == DocState.APPROVED.Value"
