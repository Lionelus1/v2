<template>
    <Dialog :header="$t('workPlan.mastersThesisInfo')" v-model:visible="showModal" :style="{width: '600px'}" :close-on-escape="true" @hide="$emit('hide')">
      ...
      <div class="p-fluid" v-if="!loading">
        <div class="field" v-for="(current, index) in fields.info" :key="index">
          ...
          :placeholder="$t('common.select')" v-model="current.value"/>
