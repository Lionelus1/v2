	...
	<div class="p-col-12 p-md-6 p-lg-4 p-fluid contact-form">
		<Panel header="Login System">
			<div class="p-grid">
				...
				<div class="p-col-12">
					<InputText type="text" v-model="loginData.username" placeholder="Username" />
				</div>
				<div class="p-col-12">
					<InputText type="password" v-model="loginData.password" placeholder="Password" />
				</div>
				<div class="p-col-12">
					<Button type="button"
						
						:disabled="loginData.username.length==0 
						|| loginData.password.length==0"
						label="LOGIN" icon="pi pi-key" @click="login"/>
				</div>
			</div>
		</Panel>
	</div>
...
import {smartEnuApi,header} from "../config/config";
export default {
	data() {
		return {
			loginData:{
	...
	methods:{
		login () {
		  ...
		  .catch(error => {
		      alert("server hate ahbarati " ,error.message);
		  });
		},
