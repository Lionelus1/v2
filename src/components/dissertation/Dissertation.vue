  ...
  <div>
    <div class="col-12">
      ...
      <Toolbar class="mb-4">
        <template #end>
          <Button v-if="isDissertationAdmin" icon="pi pi-plus" class="p-button-success mr-2"
                  v-tooltip="'Добавить'" @click="showAddCouncilDialog()"/>
          <Button v-if="isDissertationAdmin" icon="pi pi-print" class="p-button-info mr-2" @click="openNew"/>
          <Button v-if="isDissertationAdmin && selectedCouncil && !selectedCouncil?.is_closed" icon="fa-solid fa-ban" class="p-button-danger mr-2"
                  @click="showStopCouncil"
                  v-tooltip.top="$t('dissertation.closeCouncil')" :disabled="!selectedCouncil"/>
      ...
      >
        ...
        <Column :field="($i18n.locale=='kz'? 'department.nameKz' : $i18n.locale=='en'? 'department.nameEn': 'department.name')" sortable="true"
                :header="$t('common.faculty')" style="min-width:12rem"></Column>
        ...
        <div class="p-fluid">
          ...
          <div class="field">
            ...
            <PrimeCalendar style="width: 150px" :disabled="disabled" v-model="newCouncil.createdDate"
                           dateFormat="dd.mm.yy" placeholder="dd.mm.yyyy" :monthNavigator="true"
                           :yearNavigator="true" yearRange="2000:2030"/>
            <small class="p-error"
                   v-if="submitted && validationErrors.createdDate">{{ $t('dissertation.validationErrors.selectCreatedDate') }}</small>
          </div>
          <div class="field">
            <label for="open_comment">{{ $t('common.comment') }}</label>
            <Textarea id="open_comment" v-model="newCouncil.open_comment" :placeholder="$t('dissertation.councilComment')" rows="3" cols="20"
      ...
      <Dialog v-model:visible="dialog.closeCouncil.state" :style="{width: '450px'}" :header="$t('dissertation.closeCouncil')" :modal="true"
              class="p-fluid">
        <div class="p-fluid">
          <div class="field">
            <label for="name">{{ $t('dissertation.closeDate') }}</label>
            <PrimeCalendar style="width: 150px" v-model="closeCouncil.close_date"
                           dateFormat="dd.mm.yy" placeholder="dd.mm.yyyy" :monthNavigator="true"
                           :yearNavigator="true" yearRange="2000:2030"/>
            <small class="p-error"
                   v-if="submitted && closeValid.date">{{ $t('common.required') }}</small>
          </div>
          <div class="field">
            <label for="open_comment">{{ $t('common.comment') }}</label>
            <Textarea id="open_comment" v-model="closeCouncil.close_comment" :placeholder="$t('dissertation.councilComment')" rows="3" cols="20"
                      style="resize: vertical"/>
            <small class="p-error"
                   v-if="submitted && closeValid.comment">{{ $t('common.required') }}</small>
          </div>
        </div>
        <template #footer>
          <Button :label="$t('common.cancel')" icon="pi pi-times" class="p-button-text" @click="hideDialog(dialog.closeCouncil)"/>
