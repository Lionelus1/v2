      ...
      <div class="card">
        <Toolbar class="mb-4">
          <template #end>
            ...
            <Button v-if="canOpenDissertationList" icon="pi pi-list" class="p-button-primary mr-2"
              @click="openDissertationList(selectedCouncil?.id)" v-tooltip.top="$t('dissertation.dissertationList')"
            ...
            <Button v-if="canTrushDissertationList" icon="pi pi-trash" class="p-button-danger mr-2" @click="deleteCouncil()"
              :disabled="!selectedCouncil" />
        ...
        <DataTable selectionMode="single" v-model:selection="selectedCouncil" :lazy="true" :totalRecords="CouncilsCount"
          ...
          <Column headerStyle="width: 7rem; text-align: left" bodyStyle="text-align: left; overflow: visible">
            <template #body="{ data }">
              <Button v-if="!data?.is_closed" type="button" icon="pi pi-user-edit"
                @click="openCouncil(data?.id)"></Button>
            </template>
          </Column>
        ...
        <Dialog v-model:visible="dialog.addCouncil.state" :style="{ width: '450px' }" :header="$t('dissertation.title')"
          ...
          <div class="p-fluid">
            ...
            <div class="field">
              ...
              <DepartmentList :autoLoad="true" v-model="newCouncil.department"
            ...
            <div class="field">
              ...
              <Textarea id="open_comment" v-model="newCouncil.open_comment"
...
export default {
  ...
  methods: {
    ...
    openDissertationList(id) {
      if (this.isDissertationChief || this.isDissertationCouncilChief || this.isDissertationCouncilSecretary) {
  ...
  computed: {
    ...
    canOpenDissertationList() {
      return (
        (this.isDissertationAdmin || this.isDissertationCouncilChief || this.isDissertationCouncilSecretary) &&
        this.selectedCouncil &&
        !this.selectedCouncil?.is_closed
      );
    },
    canTrushDissertationList() {
      return (
        (this.isDissertationAdmin || this.isDissertationCouncilSecretary) &&
        this.selectedCouncil &&
        !this.selectedCouncil?.is_closed
      );
    },
    ...
    isDissertationChief() {
      return findRole(this.authUser, "dissertation_chief");
    },
    isDissertationCouncilChief() {
      return findRole(this.authUser, "dissertation_council_chief");
    },
    isDissertationCouncilSecretary() {
      return findRole(this.authUser, "dissertation_council_secretary");
    },
  },
}
</script>
