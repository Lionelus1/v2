<template>
  <div class="card">
    ...
    <Toolbar class="p-mb-4">
      <template #left>
        <Button v-if="(isAdmin || isModer)" :label="$t('common.add')" icon="pi pi-plus" class="p-button-success p-mr-2"
        ...
        <Button :label="$t('common.show')" v-if="selectedEvent" icon="pi pi-eye" class="p-button-secondary p-mr-2"
    ...
    <DataTable :value="allEvents"
      ...
      <Column
          :field="$i18n.locale === 'kz' ? `history.status.nameKz` : $i18n.locale === 'ru' ? `history.status.nameRu` : `history.status.nameEn`"
          v-bind:header="$t('common.status')" :sortable="true">
        <template #body="slotProps">
    ...
    <Dialog v-model:visible="editVisible" :closable="false" :style="{width: '1000px'}"
          ...
          <div class="p-grid">
            <div class="p-col-12 p-md-5">
              <FileUpload ref="form" mode="basic"
                          :customUpload="true"
                          @uploader="uploadMainImage($event)" :auto="true"
                          v-bind:chooseLabel="$t('smartenu.chooseMainImage')"></FileUpload>
            </div>
          </div>
        ...
        <div class="p-field">
          <div class="p-grid">
            <div class="p-col-12 p-md-3">
              <FileUpload ref="form" mode="basic"
                          :customUpload="true"
                          @uploader="uploadFile($event)" :auto="true"
                          v-bind:chooseLabel="$t('smartenu.chooseAdditionalFile')"></FileUpload>
            </div>
            <div class="p-col-12 p-md-5">
      ...
      <template #footer>
        <Button v-bind:label="$t('common.save')" icon="pi pi-check" class="p-button-text" v-on:click="addEvent"/>
    ...
    <Dialog v-model:visible="deleteVisible" :closable="false" :style="{width: '450px'}" :modal="true">
      ...
      <template #footer>
        <Button :label="$t('common.yes')" icon="pi pi-check" class="p-button-text" @click="deleteEvent(event.id)"/>
    ...
    <Dialog v-model:visible="eventViewVisible" :closable="false" :style="{width: '1000px'}" :modal="true"
            class="p-fluid">
      <Card style="box-shadow: none">
        ...
        <template #content>
          <div
              v-html="$i18n.locale === 'kz' ? selectedEvent.contentKz : $i18n.locale === 'ru' ? selectedEvent.contentRu : selectedEvent.contentEn">
          </div>
        </template>
        <template #footer>
          <Button v-if="selectedEvent.additionalFile" v-bind:label="$t('common.download')" icon="pi pi-download" class="p-button-link"
...
export default {
  ...
  methods: {
    ...
    getParticipantsCategories() {
      ...
      axios.get(smartEnuApi + '/getParticipantsCategories', {headers: getHeader()}).then((response) => {
     ...
     */
    deleteEvent(id) {
      axios.post(smartEnuApi + '/delEvent', {id: id}, {headers: getHeader()}).then((response) => {
    ...
    addEvent() {
      ...
      axios.post(smartEnuApi + '/addEvent', this.event, {headers: getHeader()}).then((response) => {
