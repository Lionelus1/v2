...
<div class="card">
    ...
    <Toolbar class="p-mb-4">
        <template #left>
            ...
            <Button :label="$t('common.send')" v-if="selectedNews && selectedNews.history.status.id === statuses.created && (!isModer || !isPublisher || !isAdmin)" icon="pi pi-send" class="p-mr-2" v-on:click="sendNews" />
            <Button :label="$t('common.publish')" v-if="selectedNews && (selectedNews.history.status.id === statuses.sent || selectedNews.history.status.id === statuses.created) && (isModer || isPublisher || isAdmin)" icon="pi pi-check" class="p-button-help p-mr-2" v-on:click="publishNews" />
            <Button :label="$t('common.reject')" v-if="selectedNews && selectedNews.history.status.id === statuses.sent && (isModer || isPublisher || isAdmin)" icon="pi pi-check" class="p-button-danger p-mr-2" v-on:click="rejectReason" />
            <Button :label="$t('common.show')" v-if="selectedNews" icon="pi pi-eye" class="p-button-secondary p-mr-2" v-on:click="newsView" />
        </template>
    </Toolbar>
    ...
    <DataTable :value="allNews" :paginator="true" class="p-datatable-customers" :rows="10" dataKey="id" :rowHover="true" v-model:selection="selectedNews" :filters="filters" filterDisplay="menu" :showFilterMatchModes="false" :loading="loading">
        ...
        <Column>
            <template #body="slotProps">
                <Button icon="pi pi-pencil" class="p-button-rounded p-button-success p-mr-2" @click="editNews(slotProps.data.id)" v-if="slotProps.data.history.status.id === statuses.created  || (isAdmin || isModer)" />
    ...
    <Dialog v-model:visible="rejectVisible" :style="{width: '600px'}" :header="$t('smartenu.createOrEditNews')" :modal="true" class="p-fluid">
        ...
        <template #footer>
            ...
            <Button v-bind:label="$t('common.cancel')" icon="pi pi-times" class="p-button-text" @click="rejectVisible = false" />
        </template>
    </Dialog>
    ...
    <Dialog v-model:visible="newsViewVisible" :style="{width: '1000px'}" :modal="true" class="p-fluid">
        <Card style="box-shadow: none">
            ...
            <template #content>
                <div v-html="$i18n.locale === 'kz' ? selectedNews.contentKz : $i18n.locale === 'ru' ? selectedNews.contentRu : selectedNews.contentEn">
                </div>
            </template>
            <template #footer>
                <div style="padding: 0 100px">
...
export default {
    ...
    methods: {
        ...
        getCategories() {
            this.categories = []
            axios.get(smartEnuApi + '/allCategories', {
                headers: getHeader()
            }).then((response) => {
         ...
         */
        deleteNews(id) {
            axios.post(smartEnuApi + '/delNews', {
                id: id
            }, {
                headers: getHeader()
            }).then((response) => {
