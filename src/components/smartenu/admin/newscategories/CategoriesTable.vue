...
<div class="card">
    <Toolbar class="p-mb-4">
        <template #left>
            <Button :label="$t('common.add')" v-if="isAdmin" icon="pi pi-plus" class="p-button-success p-mr-2" v-on:click="createNewsCategory" />
        </template>
    </Toolbar>
    <DataTable :value="categories" :paginator="true" class="p-datatable-customers" :rows="10" dataKey="id" :rowHover="true" v-model:selection="selectedCategory" :filters="filters" :loading="loading">
        <template #header>
        ...
        <Column>
            <template #body="slotProps">
                <Button icon="pi pi-pencil" class="p-button-rounded p-button-success p-mr-2" v-if="isAdmin" @click="editNewsCategory(slotProps.data.id)" />
                <Button icon="pi pi-trash" class="p-button-rounded p-button-warning" v-if="isAdmin" @click="delNewsCategory(slotProps.data.id)" />
            </template>
        </Column>
    </DataTable>
    <Dialog v-model:visible="editVisible" :closable="false" :style="{width: '450px'}" :header="$t('smartenu.createOrEditNewsCategory')" :modal="true" class="p-fluid">
        ...
        <div class="p-field">
            ...
            <Dropdown id="cats" v-model="category.parent" :options="categories" :optionLabel="$i18n.locale ==='kz' ? 'nameKz' : $i18n.locale === 'ru' ? 'nameRu' : 'nameEn'" v-bind:placeholder="$t('smartenu.chooseCategory')" showClear="true" :filter="true">
    ...
    <Dialog v-model:visible="deleteVisible" :closable="false" header="" :style="{width: '450px'}" :modal="true">
        ...
        <template #footer>
            <Button :label="$t('common.yes')" icon="pi pi-check" class="p-button p-component p-button-success p-mr-2" @click="deleteNewsCategory(category.id)" />
            <Button :label="$t('common.no')" icon="pi pi-times" class="p-button p-component p-button-danger" @click="deleteVisible = false" />
        </template>
    </Dialog>
</div>
</template>
...
export default {
    ...
    methods: {
        ...
        deleteNewsCategory(id) {
            axios.post(smartEnuApi + '/delNewsCat', {
                id: id
            }, {
                headers: getHeader()
            }).then((response) => {
