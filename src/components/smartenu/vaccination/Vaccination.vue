<template>
  <div>
    <div class="col-12">
          ...
          <div class="grid p-formgrid">
            ...
            <div class="col-10 p-mb-2 lg:col-3 p-mb-lg-0">
              <ToggleButton
                ...
                :class="!disabled ? 'p-button-outlined p-button-success ' : ''"
                v-model="disabled"
                :offLabel="$t('common.save')"
                :onLabel="$t('common.edit')"
                offIcon="pi pi-check"
                onIcon="pi pi-pencil"
              />
            </div>
            <div class="col-10 p-mb-2 lg:col-3 p-mb-lg-0">
              <Button
                ...
                v-if="
                  ...
                  disabled &&
                  myDetails.pasportPath
                "
                v-bind:label="$t('vaccination.pasport')"
                icon="pi pi-download"
                @click="downloadFile(myDetails.pasportPath, VS_Vaccinated)"
              />
              <FileUpload
                style="width: 210px"
                v-else-if="myDetails.status === VS_Vaccinated"
                mode="basic"
                :customUpload="true"
                @uploader="uploadPassport($event)"
                :auto="true"
                :chooseLabel="$t('vaccination.pasport')"
              ></FileUpload>
            </div>
          </div>
        </div>
        <div>
          <div class="grid p-formgrid">
            <div class="col-12 p-mb-2 lg:col-3 p-mb-lg-0 p-field grid">
              ...
              <div class="p-col vac-center">
                <Dropdown
                  :disabled="disabled"
                  style="width: 150px"
                  v-model="myDetails.status"
                  @change="Change()"
                  :options="statuses"
                >
                  <template #value="slotProps">
                    <span>{{
                      $t("vaccination.status." + statuses[slotProps.value])
                    }}</span>
                  </template>
                  <template #option="slotProps">
                    <span>{{
                      $t("vaccination.status." + statuses[slotProps.index])
                    }}</span>
                  </template>
                </Dropdown>
              </div>
            ...
            >
              ...
              <div class="p-col">
                <Dropdown
                  ...
                  @change="VaccineSelect()"
                  v-model="myDetails.vaccine"
                  :options="vaccines"
                  optionLabel="vname"
                >
                  <template #value="slotProps">
                    <span>{{
                      slotProps.value.id >= 0
                        ? slotProps.value.vname
                        : slotProps.value.vname === -1
                        ? $t("common.other")
                        : $t("common.select")
                    }}</span>
                  </template>
                  <template #option="slotProps">
                    <span>{{
                      slotProps.option.id >= 0
                        ? slotProps.option.vname
                        : slotProps.option.id === -1
                        ? $t("common.other")
                        : $t("common.select")
                    }}</span>
                  </template>
            ...
            <div class="p-col vac-center" style="align: left">
              ...
              <Button
                v-if="
                  myDetails.status === VS_Rejected &&
                  disabled &&
                  myDetails.doctorsNotePath
                "
                v-bind:label="$t('vaccination.doctorsNote')"
                icon="pi pi-download"
                @click="downloadFile(myDetails.doctorsNotePath, VS_Rejected)"
              />
              <FileUpload
                v-else-if="myDetails.status === VS_Rejected"
                mode="basic"
                :customUpload="true"
                @uploader="uploadDoctorsNote($event)"
                :auto="true"
      ...
      <div
        v-if="
          myDetails.userRole === 'dean' ||
          myDetails.userRole === 'prorector' ||
          myDetails.userRole === 'dephead' ||
          myDetails.userRole === 'vicedean' ||
          myDetails.userRole === 'vicedephead' ||
          myDetails.department ===
            'Департамент по социальному и гражданскому развитию' ||
          myDetails.department === 'Отдел социальной поддержки' ||
          myDetails.department ===
      ...
      >
        <DataTable
          :lazy="true"
          :totalRecords="detailsCount"
          :value="vaccinated"
          @page="onPage($event)"
          :paginator="true"
          class="p-datatable-vaccinated"
          :rows="10"
          dataKey="id"
          :rowHover="true"
        ...
        >
          ...
          <Column field="userRole" :header="$t('common.type')" sortable>
            ...
            <template #filter="{ filterModel }">
              ...
              >
                <template #value="slotProps">
                  <span v-if="slotProps.value" class="customer-badge">{{
                    $t("common." + slotProps.value)
                  }}</span>
                  <span v-else class="customer-badge">{{
                    slotProps.value
                  }}</span>
                </template>
                <template #option="slotProps">
          ...
          <Column :header="$t('common.department')" field="department" sortable>
            <template #body="{ data }">
              ...
              {{
                (data.facultet != null ? data.facultet + " / " : "") +
          ...
          <Column
            :header="$t('vaccination.plannedDate')"
            field="plannedDate"
            dataType="date"
          ...
          >
            <template #body="{ data }">
              <span :class="'customer-badge status-' + statuses[data.status]">{{
                $t("vaccination.status." + statuses[data.status])
              }}</span>
            </template>
            <template #filter="{ filterModel }">
              <Dropdown
              ...
              >
                <template #value="slotProps">
                  <span
                    ...
                    >{{ $t("vaccination.status." + slotProps.value) }}</span
                  >
                  <span
                    v-else
                    :class="'customer-badge status-' + slotProps.value"
                ...
                <template #option="slotProps">
                  <span :class="'customer-badge status-' + slotProps.option">{{
                    $t("vaccination.status." + slotProps.option)
                  }}</span>
                </template>
              </Dropdown>
            </template>
    ...
    <!-- Диалогтар -->
    <Dialog
      :header="$t('vaccination.fillVacTitle')"
      v-model:visible="addVaccineDialogDisplay"
    ...
    >
      ...
      <transition-group name="p-message" tag="div">
        <Message
          v-for="msg of addVaccineDialogMessage"
...
export default {
    ...
    return {
      ...
      personalData: {
        ...
        datasets: [
          ...
          {
            type: "bar",
            label: this.$t("vaccination.status.minor"),
            backgroundColor: "#9a5340",
            data: [],
          },
        ],
      },
  ...
  methods: {
    ...
    AddNewVaccine() {
      if (this.newVaccine.vname === "") {
        this.addVaccineDialogMessage = [
          {
            severity: "error",
            content: this.$t("vaccination.fillVacTitle"),
            id: 0,
          },
        ];
        return;
    ...
    getDetailsChart() {
      ...
      axios
        ...
        .then((response) => {
          ...
          response.data.forEach((el) => {
            ...
            el.role.forEach((role) => {
              if (role.name === "student") {
                ...
                role.status.forEach((status) => {
                  switch (status.id) {
                    ...
                    case this.statuses.indexOf("rejected"):
                      r = status.count;
                      break;
                    case this.statuses.indexOf("minor"):
                      m = status.count;
                      break;
                  }
                });
                ...
                this.studentData.datasets[3].data.push(m);
              }
              ...
              if (role.name === "personal") {
                ...
                this.personalData.datasets[3].data.push(m);
              }
              ...
              if (role.name === "teacher") {
                ...
                this.teacherData.datasets[3].data.push(m);
              }
            });
          });
          ...
          else this.stackedData = this.personalData;
        })
        .catch((error) => {
          if (error.response.status == 401) {
            this.$store.dispatch("logLout");
        ...
        this.$confirm.require({
          ...
          accept: () => {
            ...
            switch (this.myDetails.status) {
              case VS_NoData:
                this.myDetails = {
                  status: this.myDetails.status,
                  userId: this.myDetails.userId,
                };
                break;
              case VS_Rejected:
                this.myDetails = {
                  userRole: this.myDetails.userRole,
                  department: this.myDetails.department,
                  status: this.myDetails.status,
                  userId: this.myDetails.userId,
                  rejectReason: this.myDetails.rejectReason,
                  doctorsNotePath: this.myDetails.doctorsNotePath,
                };
                if (
                  this.myDetails.rejectReason === null ||
                  this.doctorsNoteFile === null
                ) {
                  this.$toast.add({
                    severity: "error",
              ...
              case VS_Planned:
                this.myDetails = {
                  ...
                  status: this.myDetails.status,
                  userId: this.myDetails.userId,
                  plannedDate: this.myDetails.plannedDate,
                };
                if (this.myDetails.plannedDate === null) {
                  this.$toast.add({
                    severity: "error",
              ...
              case VS_FirstComponent:
                this.myDetails = {
                  ...
                  userId: this.myDetails.userId,
                  plannedDate: this.myDetails.plannedDate,
                  date1: this.myDetails.date1,
                  vaccine: this.myDetails.vaccine,
                  clinic: this.myDetails.clinic,
                };
                if (
                  this.myDetails.vaccine.id < 0 ||
                  this.myDetails.date1 === null ||
                  this.myDetails.date1 === "" ||
                  this.myDetails.vaccine.id === null ||
                  this.myDetails.clinic === null
                ) {
                  this.$toast.add({
                    severity: "error",
                    summary: this.$t("common.message.fillError"),
              ...
              case VS_Vaccinated:
                ...
                if (
                  this.myDetails.vaccine.id < 0 ||
                  this.myDetails.date2 === null ||
                  this.myDetails.date2 === "" ||
                  this.myDetails.vaccine.id === null ||
                  this.myDetails.clinic === null ||
    ...
    ChangeStackData() {
      if (this.selectedData === "student") this.stackedData = this.studentData;
      else if (this.selectedData === "teacher")
        this.stackedData = this.teacherData;
      else this.stackedData = this.personalData;
    },
    uploadPassport(event) {
      if (event.files[0].type.includes("image")) {
        imageResizeCompress
          .fromBlob(event.files[0], 90, 720, "auto", "jpeg")
          .then((res) => {
            this.passportFile = res;
          });
      } else {
