<template>
  <div class="p-field p-grid">
    <label  class="p-col-12 p-md-2">{{$t('doctemplate.editor.contragent')}}</label>
    <div class="p-col-12 p-md-10 p-mb-2">
      <span class="p-float-label p-ibutoon-right">
        <i v-if="value" class="pi pi-id-card ibutton" style="margin-right:35px;height:32px;margin-top: 2px;" @click="showcard()"/>
        <i class="pi pi-ellipsis-h ibutton" style="height:32px;margin-top: 2px;margin-right: 2px;" @click="showside()"/>
    ...
    <div v-if="value && value.type != ContragentType.Person" class="p-col-12 p-md-10">
      <span class="p-float-label p-ibutoon-right">
        <i v-if="value && value.data && value.data.signer" class="pi pi-id-card ibutton" style="margin-right:35px;height:32px;margin-top: 2px;" @click="showcard('person')"/>
        <i v-if="value" class="pi pi-ellipsis-h ibutton" style="height:32px;margin-top: 2px;margin-right: 2px;" @click="showside('person')"/>
    ...
    selectedContragentName() {
      ...
      switch(this.value.type) {
        case Enum.ContragentType.Organization:
          return this.$i18n.locale != 'ru' ? '"' + this.value.data.name + '" ' + this.value.data.form.shortname : this.value.data.form.shortnamerus + ' "' + this.value.data.namerus + '"';
        ...
        case Enum.ContragentType.Bank:
          return this.$i18n.locale != 'ru' ? '"' + this.value.data.organization.name + '" ' + this.value.data.organization.form.shortname : this.value.data.organization.form.shortnamerus + ' "' + this.value.data.organization.namerus + '"'
      }
