...
<div>
  <div class="content-section introduction">
	...
	<div class="card p-p-0">
      ...
      <div class="box">
				<DataTable class="p-datatable-sm" v-model:selection="selectedOrganizations" selectionMode="single" dataKey="id" :value="organizations" :paginator="true" :rowHover="true" :rows="orgShowCount" :filters="filters"
				:loading="loading">
					<template #empty>
            	...
            	<Button class="p-button-link p-text-left" @click="toggle($event,slotProps.data)">{{(this.$i18n.locale != 'ru' ? '"' + slotProps.data.name + '" ' + slotProps.data.form.shortname : slotProps.data.form.shortnamerus + ' "' + slotProps.data.namerus + '"')}}</Button>
        		</template>
					</Column>
					<Column field="iin" :header="$t('contact.bin')" :sortable="true"></Column>
					<Column field="locality.name" :header="$t('contact.locality')" :sortable="true"></Column>
