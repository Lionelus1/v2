...
<script setup>
 import {getHeader,  smartEnuApi} from "@/config/config";
    ...
    const update = () => { 
      ...
      fd.append("id", JSON.stringify(user.value))
    ...
    const validateForm = () => {
        validation.value.number = !user.value.idnumber || user.value.idnumber == ""
        validation.value.startDate = !user.value.iddate || user.value.iddate == ""
        validation.value.issuedBy = !user.value.idsourceStr || user.value.idsourceStr == ""
        validation.value.iin = !user.value.iin || user.value.iin == ""
