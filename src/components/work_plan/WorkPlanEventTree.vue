  ...
  <DataTable :lazy="true" :value="data" dataKey="work_plan_event_id"
    ...
    <Column field="actions" header="">
      <template #body="slotProps">
        ...
        <work-plan-event-add v-if="(slotProps.data.creator_id === loginedUserId || isPlanCreator) && !isPlanSentApproval && !slotProps.data.is_finish" :data="slotProps.data" :items="data" :plan-data="plan"></work-plan-event-add>
        ...
        <div>
          <Button v-if="(slotProps.data.creator_id === loginedUserId || isPlanCreator) && !isPlanSentApproval && !slotProps.data.is_finish" @click="remove_event(slotProps.data.work_plan_event_id)" icon="pi pi-trash" class="p-button-danger ml-1 mt-1"></Button>
        </div>
      </template>
    </Column>
