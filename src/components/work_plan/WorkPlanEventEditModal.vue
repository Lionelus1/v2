<template>
  <Dialog :header="$t('workPlan.editEvent')" v-model:visible="showWorkPlanEventEditModal" :style="{ width: '450px' }"
    ...
    <div v-if="plan?.plan_type?.code !== Enum.WorkPlanTypes.Internship">
      <div class="field">
        <!-- --- -->
      ...
      <div class="field" v-if="!isShedulePlan && !isEditResponsiveUsers">
        <label>{{
            plan && plan.plan_type.code === Enum.WorkPlanTypes.Oper ? $t('common.additionalInfo') : $t('common.result')
          }}</label>
    ...
    <div v-if="plan?.plan_type?.code === Enum.WorkPlanTypes.Internship">
      ...
      <div class="field" v-if="plan?.plan_type?.code !== Enum.WorkPlanTypes.Internship">
        ...
        <Dropdown v-model="editData.semester" :options="semesters" optionLabel="name" optionValue="id"
