<template>
  <Dialog :header="$t('workPlan.editEvent')" v-model:visible="showWorkPlanEventEditModal" :style="{ width: '450px' }" class="p-fluid" @hide="closeBasic">
    ...
    <div
        v-if="(plan?.plan_type?.code === Enum.WorkPlanTypes.Masters || plan?.plan_type?.code ===Enum.WorkPlanTypes.Doctors) && plan?.plan_type?.code !== Enum.WorkPlanTypes.WorkSchedule">
      <div class="field">
